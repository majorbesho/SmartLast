...
class ProductController extends Controller
{
    public function index()
    {
        ...
        $GroupProducts = groupProduct::where('status', 'active')->orderBy('id', 'DESC')->get();
        return response()->json([
            'status' => "success",
            'data' => ProductResource::collection($GroupProducts),
        ], 200);
    }
    ...
    public function pay(Request $request)
    {
        $order_id = $request->query("order_id");
        if (empty($order_id)) {
            return Redirect::to('https://smartboxuae.ae/inapp/checkout/pending-payment?order_id=' . $order_id);
        }
        ...
        $checkout_session = \Stripe\Checkout\Session::create([
            ...
            'mode' => 'payment',
            'success_url' =>  'https://smartboxuae.ae/inapp/checkout/stripe?success=true',
            'cancel_url' => 'https://smartboxuae.ae/inapp/checkout/pending-payment?order_id=' . $order_id . '&canceled=true',
        ]);
    ...
    public function shareProduct (Request $request, $slug){
        ...
        $product = groupProduct::with('products')->where(['slug' => $slug])->first();
        //dd($product);
        if(!$product){
            return Redirect::to('https://smartboxuae.ae');
        }
